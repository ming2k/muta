...
//!
//! [`Message`]: muta_contracts::Message
...
pub(super) fn transcript_message_from_core(message: Message) -> Option<TranscriptMessage> {
    ...
    let is_insert = message
        .origin
        .as_ref()
        .is_some_and(|origin| origin.kind == muta_contracts::InjectionKind::UserSteer);
...
pub(super) fn transcript_interrupts_from_records(
    records: Vec<muta_contracts::RoundInterrupt>,
) -> Vec<TranscriptMessage> {
    records
        .into_iter()
        .map(|record| {
    ...
    for mut message in messages {
        ...
        if message.role == Role::Assistant {
            ...
            let chain_disclosed = model
                .as_deref()
                .and_then(muta_contracts::model_by_id)
...
mod tests {
    ...
    fn restores_interrupted_subagent_with_children_and_status() {
        ...
        use muta_contracts::message::SubagentMeta;
