...
pub(crate) fn render_body(
    ...
    if wrap {
        para = para.wrap(mutx_engine::Wrap { trim: false });
    }
...
fn cell_at_index(
    buf: &mut mutx_engine::Grid,
    area: Rect,
    x: u16,
    y: u16,
) -> &mut mutx_engine::Cell {
...
mod tests {
    ...
    fn dim_surface_preserves_more_code_text_contrast() {
        ...
        let mut grid = mutx_engine::Grid::new(2, 1);
        grid.set(
            0,
            0,
            mutx_engine::Cell::narrow(
                "c",
                Style::default()
                    .fg(theme.code_text())
        ...
        grid.set(
            1,
            0,
            mutx_engine::Cell::narrow("p", Style::default().fg(theme.fg()).bg(theme.surface())),
        );
