...
pub fn draw_input_injection(
    frame: &mut Frame,
    request: &muta_contracts::InputRequest,
    input: &str,
    _cursor: usize,
    input_rect: Rect,
    theme: &Theme,
) -> Rect {
    use mutx_engine::Layout;
    // Split the composer rect into a prompt row + the input row(s).
    let chunks = Layout::default()
        .direction(mutx_engine::Direction::Vertical)
        .constraints([
            mutx_engine::Constraint::Length(1),
            mutx_engine::Constraint::Min(0),
        ])
        .split(input_rect);
...
mod tests {
    use super::*;
    use muta_contracts::{UserQuestion, UserQuestionOption};
    ...
    fn sheet_text(terminal: &mutx_engine::TestTerminal) -> String {
