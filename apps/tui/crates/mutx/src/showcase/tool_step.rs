...
//!
//! `mutx showcase tool-step` paints a fixed conversation whose turn is a
...
fn draw_title(f: &mut mutx_engine::Frame, text: &str, theme: &Theme) {
    use mutx_engine::{Line, Rect, Span, Style};
    let area = f.area();
    let row = Rect::new(area.x, area.y, area.width, 1);
    let line = Line::from(Span::styled(text, Style::default().fg(theme.muted())));
    f.render_widget(
        mutx_engine::Paragraph::new(line).style(Style::default().bg(theme.surface())),
        row,
    );
}
