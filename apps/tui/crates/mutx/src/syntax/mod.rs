...
pub fn tokenize_line(line: &str, lang: Language) -> Vec<SyntaxSpan> {
    ...
    while i < len {
        ...
        // 1. Comments
        if matches!(lang, Language::Rust | Language::TypeScript | Language::JavaScript | Language::Go | Language::Css)
            && i + 1 < len
            && b == b'/'
            && bytes[i + 1] == b'/'
        {
        ...
        if matches!(lang, Language::Python | Language::Shell | Language::Toml | Language::Yaml)
            && b == b'#'
        {
            spans.push(SyntaxSpan {
                start_byte: i,
        ...
        // 2. Strings
        if b == b'"' || b == b'\'' || (b == b'`' && matches!(lang, Language::JavaScript | Language::TypeScript | Language::Go | Language::Markdown | Language::Shell)) {
            let quote = b;
            let start = i;
            i += 1;
            while i < len {
...
fn classify_ident(word: &str, lang: Language, is_fn: bool) -> SyntaxKind {
    ...
    if matches!(word, "true" | "false" | "null" | "None" | "nil" | "undefined" | "NaN" | "Some" | "Ok" | "Err") {
    ...
    let is_kw = match lang {
        ...
        Language::Shell => matches!(
            ...
            "if" | "then" | "else" | "elif" | "fi" | "case" | "esac" | "for" | "while"
                | "until" | "do" | "done" | "in" | "function" | "return" | "exit" | "export" | "local"
        ),
    ...
    if matches!(
        ...
        "i8" | "i16" | "i32" | "i64" | "i128" | "isize" | "u8" | "u16" | "u32" | "u64"
            | "u128" | "usize" | "f32" | "f64" | "str" | "bool" | "char" | "String" | "Vec"
            | "Option" | "Result" | "int" | "float" | "boolean" | "number" | "string" | "any"
            | "void"
    ) {
        return SyntaxKind::Type;
    }

    if word.starts_with(|c: char| c.is_ascii_uppercase()) && !word.chars().all(|c| c.is_ascii_uppercase() || c == '_') {
        return SyntaxKind::Type;
    }
...
mod tests {
    ...
    fn test_tokenize_rust_line() {
        ...
        let kw_pub = tokens.iter().find(|t| &line[t.start_byte..t.end_byte] == "pub").unwrap();
        assert_eq!(kw_pub.kind, SyntaxKind::Keyword);

        let kw_fn = tokens.iter().find(|t| &line[t.start_byte..t.end_byte] == "fn").unwrap();
        assert_eq!(kw_fn.kind, SyntaxKind::Keyword);

        let fn_calc = tokens.iter().find(|t| &line[t.start_byte..t.end_byte] == "calculate").unwrap();
        assert_eq!(fn_calc.kind, SyntaxKind::Function);

        let type_i32 = tokens.iter().find(|t| &line[t.start_byte..t.end_byte] == "i32").unwrap();
        assert_eq!(type_i32.kind, SyntaxKind::Type);

        let type_str = tokens.iter().find(|t| &line[t.start_byte..t.end_byte] == "String").unwrap();
    ...
    fn test_tokenize_strings_and_comments() {
        ...
        let str_tok = tokens.iter().find(|t| t.kind == SyntaxKind::String).unwrap();
        assert_eq!(&line[str_tok.start_byte..str_tok.end_byte], "\"hello world\"");

        let comment_tok = tokens.iter().find(|t| t.kind == SyntaxKind::Comment).unwrap();
        assert_eq!(&line[comment_tok.start_byte..comment_tok.end_byte], "// greeting");
    }
}
