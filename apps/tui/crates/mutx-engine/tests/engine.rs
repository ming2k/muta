...
use mutx_engine::diff;
use mutx_engine::{Cell, Color, Fit, Grid, Pos, Style};
...
fn wide_glyph_trailing_column_carries_background_not_reset() {
    ...
    let style = Style::default()
        ...
        .bg(Color::Rgb(7, 8, 8)); // near-black surface, like muta's theme
...
fn bce_clear_to_end_uses_clr_eol_only_for_default_bg() {
    ...
    use mutx_engine::diff::{Draw, DrawCmd};
    crossterm::style::force_color_output(true);
    let cmd = DrawCmd {
        w: 10,
        h: 10,
...
impl ModelTerminal {
    ...
    fn apply(&mut self, bytes: &str) {
        ...
        while let Some(c) = it.next() {
            ...
            let g = c.to_string();
            let width = mutx_engine::text::grapheme_width(&g).max(1) as u16;
            if self.cx < self.w {
