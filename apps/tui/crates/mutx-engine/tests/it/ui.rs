...
fn root(ui: &mut UiRuntime<&'static str>) {
    ui.begin(VIEW);
    ui.mount(Component::new("root", None, LayoutBox::Fill)).unwrap();
}

fn child(ui: &mut UiRuntime<&'static str>, key: &'static str, rect: Rect) {
    ui.mount(Component::new(key, Some("root"), LayoutBox::Placed(rect)).interactive()).unwrap();
}
...
fn ui_modal_restores_focus_and_releases_covered_pointer_capture() {
    ...
    root(&mut ui);
    child(&mut ui, "editor", Rect::new(0, 15, 40, 5));
    ui.mount(Component::new("modal", Some("root"), LayoutBox::Placed(Rect::new(10, 5, 20, 10))).modal().layer(10)).unwrap();
...
fn ui_clipping_and_viewport_attachments_have_distinct_geometry() {
    ...
    ui.mount(Component::new("clipped", Some("parent"), LayoutBox::Placed(Rect::new(0, 0, 20, 10))).interactive()).unwrap();
    ui.mount(Component::new("popup", Some("parent"), LayoutBox::Viewport(Rect::new(0, 0, 4, 4))).interactive().layer(1)).unwrap();
    ui.commit().unwrap();
    assert_eq!(ui.presented().layout(&"clipped").unwrap().clip, Rect::new(5, 5, 10, 5));
...
fn ui_removal_repaints_old_coverage_and_preserves_clean_overlays() {
    ...
    let paint = |key: &&str, layout: mutx_engine::ui::NodeLayout, f: &mut Frame<'_>| {
        let ch = match *key { "root" => '.', "moving" => 'm', _ => 'o' };
        for y in layout.bounds.y..layout.bounds.bottom() {
            f.put(layout.bounds.x, y, Style::default(), &ch.to_string().repeat(layout.bounds.width as usize));
        }
    };
    root(&mut ui);
    child(&mut ui, "moving", Rect::new(0, 0, 5, 2));
    ui.mount(Component::new("overlay", Some("root"), LayoutBox::Placed(Rect::new(3, 0, 4, 2))).layer(1)).unwrap();
...
fn ui_invalid_structure_is_rejected_without_publishing_it() {
    let mut ui = UiRuntime::default();
    root(&mut ui);
    assert_eq!(ui.mount(Component::new("root", None, LayoutBox::Fill)), Err(UiError::DuplicateKey));
    assert_eq!(ui.mount(Component::new("orphan", Some("absent"), LayoutBox::Fill)), Err(UiError::MissingParent));
...
fn ui_painter_cannot_escape_clip_even_through_direct_buffer_access() {
    ...
    Frame::new(&mut grid).paint_clipped(Rect::new(1, 1, 3, 1), |f| {
        for y in 0..3 { f.put(0, y, Style::default(), "abcdef"); }
    });
