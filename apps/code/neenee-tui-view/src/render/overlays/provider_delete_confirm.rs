...
use crate::render::primitives::{modal_frame, recess_backdrop, viewport_rect};

/// Which button in the confirm overlay holds keyboard focus. Mirrors
/// `neenee_code::tui::app::ProviderDeleteChoice` but lives in the view crate
/// (which cannot depend on the app shell) so the renderer stays decoupled.
