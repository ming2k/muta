...
- **Terminal Model Release Compilation:**
  - Removed  attribute from  on , resolving release-mode build failure () when referenced by the PreAttach trust gate footer.
...
- Replaced legacy activity and pulse overlays with a streamlined todos modal and integrated layout mapping.
- Enhanced mouse interaction and selection state handling across document render flows.
- Standardized web tool interfaces and configuration options across runtime and agents.
...
- Support for provider continuation state and conversation revisions across provider catalog (OpenAI Responses, Anthropic, Google protocols).
- Reusable dropdown / popover component for TUI (`mutx`).
