...
muta uses the public Copilot OAuth App client id (`Ov23li8tweQw6odWQebz`)
...
| Prefix | Source | How muta uses it |
|--------|--------|--------------------|
| `gho_` | GitHub OAuth App token (the default for muta's built-in client id) | Sent directly as `Authorization: Bearer <token>`. Works for Individual Copilot subscribers and the GA model catalog. |
| `ghu_` | GitHub App user token (for example from VS Code's Copilot extension client id `Iv1.b507a08c87ecfe98`) | Must be exchanged through `GET /copilot_internal/v2/token` for a short-lived bearer before calling the Copilot API. muta currently does not perform this exchange, so `ghu_` tokens will not work correctly. |

If you paste a token obtained from VS Code or the GitHub CLI into muta, it
...
`/copilot_internal/v2/token` exchange. muta currently hardcodes
...
includes internal utility models, embeddings, and preview entries. muta keeps
...
1. **Re-run device login.** Delete the copilot entry from `auth.toml` and log in
   ...
   muta's built-in public client id.
...
5. **Check the account plan.** Business/Enterprise plans may need a different
   API host and token exchange, which muta does not yet support.
