...
`crates/muta-contracts/src/tool_output.rs`. `run_command` is the reference example — it
...
tools use the `path` argument, `run_command` uses the full `command` text. Pick a scope that distinguishes
