...
hundreds of lines of `run_command` logs, whole files from `read`, search dumps, a
subagent's full transcript. These age badly — an early `search_text` result rarely
...
Nested subagent transcripts descend the same policy: a `subagent` result carries
