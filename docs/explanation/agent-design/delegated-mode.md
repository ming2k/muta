...
Unattended execution is an interaction posture: the agent runs
...
The posture was previously called **autopilot** / **yolo**, and later **delegated**.
Under first-principles semantic alignment, the term *delegation* was strictly returned
to multi-agent task dispatch (subagent delegation, ADR-0183), while human interaction
posture was cleaned into **unattended execution** (`/unattended`, `--unattended`, the
`UNATTENDED` head-row badge), pairing cleanly with **attended execution**.
For backward compatibility in slash commands, `/auto` and `/delegate` remain accepted
aliases in the command harness.
...
| Authority result | Attended | Unattended |
|------------------|----------|------------|
...
Unattended mode never auto-approves the broker, converts a confirmation into an
...
Unattended mode consults the same three planes as an attended session. It does
...
Unattended mode reclaims the surfaces that would otherwise park a round:
