...
4. Always requires a separate confirmation and is cached by tool plus resource
   scope for the current process. File writes scope by path and run_command scopes by
...
The delegated toggle suppresses this broker entirely: when on, a
...
[Delegated autonomous execution](delegated-mode.md).
