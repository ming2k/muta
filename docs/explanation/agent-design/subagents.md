...
The `spawn_agent` tool spawns an isolated child agent to investigate a sub-question
...
model spends turns re-reading things it already saw. An subagent gives the model
...
2. **Capability-bounded by construction.** The subagent receives only the tools
   its profile admits, scoped by a per-agent `OperationScope`, so it can only do
...
The `spawn_agent` tool is the one built-in tool whose result is not a single value
...
| Write boundary | No | A `OperationScope` resolved from the profile's `write_paths` grant |
...
`OperationScope` boundary (ADR-0028).

## Profiles

An subagent's behaviour is not hardcoded in the dispatch tool. It is the output
...
delegated task, so the child's writes and commands execute on the subagent's
...
subagent's handoff (files touched, commands run, verification) and stays
...
tool may run; the `OperationScope` (for a future scoped profile) says *where*.
...
- **Scoped writes, not blanket writes.** A `Write` tool admitted by
  `write_paths` is then scoped at runtime by the agent's `OperationScope`, so a
  write anywhere outside the granted path is blocked at the execution funnel.
  Admission says *whether*; `OperationScope` says *where*. No built-in profile
  uses this today.
- **Todo tools are inert.** They are added inside the subagent
...
An subagent is **not** fire-and-forget. Communication is full-duplex
...
- **Down.** Each child installs a steering inbox and exposes an `SubagentHandle`
  (`Weak<Agent>` + a sender). The dispatch tool keeps an `SubagentRegistry`
...
subagent meaning are dropped on the way through.
...
An subagent that hits a harness safety bound (a read-only stall, repeated
...
The subagent runs with its own independent cancellation. When the parent round is
...
Real token usage from the subagent is accumulated into the parent round's cost.

## See also

- [`subagent`](../../reference/tools/subagent.md) — parameter reference.
- [Rounds and turns](rounds-and-turns.md) — the request/response cycle the subagent runs
  internally.
...
- [ADR-0011](../../adr/0011-subagent-profiles.md) — the capability-axis
  profile primitive.
- [ADR-0028](../../adr/archive/0028-capability-allocation-scoped-writes.md) — the
  `OperationScope` grant.
