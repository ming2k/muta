...
instructions, and the model then uses ordinary tools (run_command, edit, ...) to do
...
1. **Explicit — `use_skill`.** The model calls the `use_skill` tool with a
   ...
   ordinary read-only tool, architecturally identical to `read_text` or `run_command`;
