...
- **Status:** Superseded by ADR-0073 (the `neenee-quant` application was removed) and ADR-0075 (the `neenee-cli` → `neenee-code` rename is reversed, restoring the bare `neenee` binary name)
- **Date:** 2026-06-26
- **Archived:** 2026-09-08
...
> **Notice**: Superseded by [ADR-0073](../0073-flat-coding-focused-workspace.md). This record was
> relocated to cold storage (`docs/adr/archive/`) on 2026-09-08. It is preserved
> for historical rationale and negative-knowledge audit only; the binding rule
> lives in the superseding record.
...
[`register_tool!`](../../../crates/muta-contracts/src/tool_registry.rs) so the coding
