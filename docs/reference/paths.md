...
| Path | Purpose | Lossy? |
|------|---------|--------|
| `config.toml` | User-edited configuration — **daemon & core behavior only** (`default_connection` / `default_model`, `[compaction]`, `[permissions]`, `[workspace]`, `[bash_policy]`, `[tool_variants]`, `[[hooks]]`, `[skills]`, `[web]`, `[mcp.<server>]`, `[daemon]`, `[agent]`, ...). Model connections live in `connections.toml`; secrets live in `credentials.toml` | Yes |
| `roles.toml` | User-authored custom roles and capability scopes (`[roles.<id>]`, `tools`, `admit_mcp` patterns, workspace policies; ADR-0242, ADR-0246) | Yes (user-authored) |
