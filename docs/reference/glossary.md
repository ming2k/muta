...
| **remote catalog** | Single-source pluggable remote catalog sync (`RemoteCatalogSource`) replacing legacy discovery and fitting; yields model candidates and sparse capability facts overlaid onto the compiled baseline via tristate merge. [ADR-0203](../adr/0203-remote-catalog-overlay-and-connection-gated-pipeline.md) |
| **connection pipe** | The credentialed named pipe connecting to a model provider, admitting or excluding models through deterministic valve algebra: `(Filter(C, filter) ∪ inject) \ block`. [ADR-0203](../adr/0203-remote-catalog-overlay-and-connection-gated-pipeline.md) |
...
| **`mutx`** | The terminal app package and binary under `apps/terminal`: owns interactive/headless prompt clients, attachment, dashboard, clipboard behavior, rendering, and `mutx-engine`. [ADR-0136](../adr/0136-muta-core-and-mutx-terminal-app.md) |
...
| **session daemon** | The single user-level daemon runtime (`muta-runtime`) that owns all sessions across all projects, started on demand by `mutx` or via `muta start`. [ADR-0096](../adr/0096-unified-session-daemon.md), [ADR-0136](../adr/0136-muta-core-and-mutx-terminal-app.md) |
