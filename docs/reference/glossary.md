...
| **`neenee-cli`** | The package producing the `neenee` command; contains the TUI and the CLI verbs (`serve` / `attach` / `status`). Every invocation is a client of the unified session daemon — there is no in-process standalone session (ADR-0096). The *coding* application's interactive frontend. [ADR-0075](../adr/0075-rename-neenee-code-to-neenee.md), [ADR-0080](../adr/0080-rename-neenee-to-neenee-cli.md), [ADR-0096](../adr/0096-unified-session-daemon.md) |
| **`neenee-server`** | The unified session-daemon binary: owns every session across every project and serves them over the control plane (UDS by default, TCP + bearer token when exposed). Run by `neenee serve` / `--detach`. [ADR-0096](../adr/0096-unified-session-daemon.md) |
...
| **session daemon** | The single user-level process (`neenee-server`) that owns all sessions across all projects, started on demand or via `neenee serve`. [ADR-0096](../adr/0096-unified-session-daemon.md) |
...
| **`/dashboard`** | The TUI session dashboard: a first-class, full-screen live view over every daemon session — a session list plus a detail pane; Enter attaches via detach + attach (never killing running work), and `i` / `p` / `n` interrupt / prompt / create. `/host` is a hidden alias. [ADR-0096](../adr/0096-unified-session-daemon.md) |
