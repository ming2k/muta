...
| **`/host`** | The TUI control panel: a live view over every daemon session with per-row status and a preview; Enter switches sessions via detach + attach (never killing running work). [ADR-0096](../adr/0096-unified-session-daemon.md) |
