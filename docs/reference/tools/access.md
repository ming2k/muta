...
and the subagent admission policy lives in
[Subagent profiles](../../explanation/agent-design/subagents.md#profiles) and
...
| `Execute` | Admitted only above a `Read` ceiling | Prompted unless a cached `Always` rule matches | `run_command` |
...
| Affects process control | `affects_control_flow()` (default `false`) | Subagent profiles, broker bypass | — |
