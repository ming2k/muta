...
(`crates/platform/neenee-mcp/src/client.rs`) and dispatch `tools/call` JSON-RPC over
...
`{"type":"object"}` when absent (`crates/platform/neenee-mcp/src/client.rs`). The public
