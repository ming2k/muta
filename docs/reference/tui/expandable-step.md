...
in `apps/code/neenee-tui-view/src/render/disclosure/renderers.rs`. Sticky-pin tracking:
`StickyStep` in the same module. `BlockRegion` is defined in
`apps/code/neenee-tui-view/src/layout.rs`. The structured output the tool-step body
