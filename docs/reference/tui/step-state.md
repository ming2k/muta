...
through one shared state model in `apps/terminal/crates/mutx/src/disclosure/`.
...
| Tool | `Ok` | per-tool `[tui.default_expanded]` entry, or `true` under Comfortable density | `edit_text` shows its diff; `run_command` / `read_text` stay collapsed |
