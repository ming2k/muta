...
[mutx-engine](../../../apps/terminal/crates/mutx-engine/src/lib.rs) engine (ADR-0038) is a
...
(`apps/terminal/crates/mutx/src`) owns `App` state, the event loop, and input
...
other. View and shell files both live under `apps/terminal/crates/mutx/src/` since
ADR-0079; paths below are relative to that directory.

### View layer — `apps/terminal/crates/mutx/src/` (view modules)
...
| `tools/` | Per-tool-step presenters (`run_command`, `edit`, `read`, `search`, `web`, `ask_user`, `read_image`, `diff`, `meta`, `fallback`) |
| `composer.rs` | `draw_composer` (live input box), `INPUT_MSG_IDX` |
| `chrome.rs` | `draw_activity_bar` (breathing dot + status + elapsed), `draw_queue_bar` (outbox summary), `draw_model_bar` (ambient gauges below the composer), `draw_completion_menu` |
...
### App shell — `apps/terminal/crates/mutx/src/` (shell modules)
