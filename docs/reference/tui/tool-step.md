...
| `read_text`, others | `draw_code_content` | Code block with line-number gutter on `code_bg` (the fallback for unrecognized tools), rendered with lexical syntax highlighting based on detected file language. |
