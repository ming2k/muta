...
`draw_tool_step` in `apps/code/neenee-tui-view/src/render/disclosure/renderers.rs`. Shared
header via `draw_expandable_step_header` (from
`apps/code/neenee-tui-view/src/render/disclosure/mod.rs`). Expanded content dispatched by
...
`arg_layout`) live in `apps/code/neenee-tui-view/src/render/tools/`. The structured
