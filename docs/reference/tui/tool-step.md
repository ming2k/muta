...
`draw_tool_step` in `crates/neenee-tui-view/src/disclosure/renderers.rs`. Shared
header via `draw_expandable_step_header` (from
`crates/neenee-tui-view/src/disclosure/mod.rs`). Expanded content dispatched by
...
`arg_layout`) live in `crates/neenee-tui-view/src/tools/`. The structured
