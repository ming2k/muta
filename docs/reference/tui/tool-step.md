...
`draw_tool_step` in `crates/neenee-cli/src/tui/disclosure/renderers.rs`. Shared
header via `draw_expandable_step_header` (from
`crates/neenee-cli/src/tui/disclosure/mod.rs`). Expanded content dispatched by
...
`arg_layout`) live in `crates/neenee-cli/src/tui/tools/`. The structured
