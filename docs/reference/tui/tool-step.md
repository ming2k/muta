...
`draw_tool_step` in `crates/neenee-tui/src/disclosure/renderers.rs`. Shared
header via `draw_expandable_step_header` (from
`crates/neenee-tui/src/disclosure/mod.rs`). Expanded content dispatched by
...
`arg_layout`) live in `crates/neenee-tui/src/tools/`. The structured
