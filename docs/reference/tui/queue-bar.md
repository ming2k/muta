...
```text
 QUEUE 1  fix the flaky test in the parser   Ctrl+P block  Ctrl+Q expand
```
...
outbox), and the right-pinned **keycap legend** (`Ctrl+P` block/resume,
`Ctrl+Q` expand). The keycap units are
...
(keeping the bare keycaps `Ctrl+P  Ctrl+Q`), then the `Ctrl+Q`
cluster drops (keeping just `Ctrl+P`, the state toggle), then the whole legend —
...
are `muta_tui::app::QueuedDispatch`, mirrored into the view via
`QueueItemProps`. The block state lives in `App::queue_blocked_sessions`; the
