...
```text
 QUEUE 1                     F3 block  F2 expand
 fix the flaky test in the parser
```

Row 1 carries the identity: a brand-colored **`QUEUE`** label, the **item
count**, and a right-pinned **keycap legend** (`F3` block/resume, `F2`
expand). The two keycap units are same-rank peers, so they join with plain
whitespace (R2 in the [join ladder](visual-language.md)) — no `·`. Row 2 is a
one-line **preview of the next item to pop** (front of the FIFO outbox),
truncated with `…`.
...
from the identity. Under width pressure the right legend degrades in a fixed
order: the `expand` label drops (keeping `F3 block · F2`), then the
`block`/`resume` label (keeping the bare keycaps `F3 · F2`), then the `F2`
cluster (keeping just `F3`), then the whole legend — so the identity on the
left always survives. The preview truncates to the remaining width.
