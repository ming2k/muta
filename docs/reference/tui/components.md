...
| `components/meta_strip.rs` | `MetaStrip`, `MetaChip`, `MetaTone` | Turn header (`layout_default`), sent/queued user-message header (`message_body`) | One-row two-tone metadata strip (accent anchor · muted details), with optional left padding and background tail fill |
