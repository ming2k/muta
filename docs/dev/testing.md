...
why `--home` / `NEENEE_HOME` is mandatory and how the sandbox is
...
the isolation rules every suite relies on (`--home`, `NEENEE_HOME`,
`test-path-override`).
...
| Snapshot | `crates/neenee-tui/src/snapshot_tests.rs` → `src/snapshots/*.snap` (`insta`) | Rendered TUI frames: tool step expansion, diffs, question modals |
| Doc tests | Rust fences in doc comments (sparse) | Examples in `neenee-tui-engine` and `neenee-contracts` compile |
...
Two tests in `crates/neenee-agent/tests/webtool_e2e.rs` are
`#[ignore = "live network"]` — they hit real endpoints and run only when
explicitly selected:

```bash
cargo test -p neenee-agent --test webtool_e2e -- --ignored
```

## Run tests

`cargo test` without a selector runs the default workspace member
(`neenee-cli`). Scope to what you changed:

```bash
cargo test -p neenee-agent                      # one crate
cargo test -p neenee-tui snapshot_tests         # one module substring
cargo test -p neenee-runtime --test serve_integration   # one integration file
cargo test -p neenee-agent --test duplex streaming_loop  # one test by name substring
```

The full suite, as CI runs it:

```bash
export NEENEE_HOME=$(mktemp -d /tmp/neenee-test.XXXXXX)
cargo test --workspace --locked --no-fail-fast
./target/debug/neenee daemon stop    # if any suite spawned a daemon
rm -rf "$NEENEE_HOME"
```

`--no-fail-fast` mirrors CI so one failure does not hide the rest. Always
export `NEENEE_HOME` for whole-suite runs: the variable carries the
...
```bash
RUST_BACKTRACE=1 cargo test -p neenee-runtime --test lifecycle_integration
cargo test -p neenee-agent -- --nocapture
```
...
```bash
cargo build -p neenee-cli --locked
bash apps/web/e2e/cli-smoke.sh

mkdir -p /tmp/neenee-e2e-home
NEENEE_HOME=/tmp/neenee-e2e-home NEENEE_PORT=9800 \
  ./target/debug/neenee daemon start --port 9800 &
TOKEN=$(node -p "JSON.parse(require('fs').readFileSync(\
'/tmp/neenee-e2e-home/neenee/instance/daemon.json','utf8')).token")
DAEMON_TOKEN="$TOKEN" node apps/web/e2e/daemon-smoke.mjs
./target/debug/neenee daemon stop
```
...
|------|------------------------|----------|-----------|
| Session harness, serve transport | `neenee-runtime` inline tests; `serve_integration`, `lifecycle_integration`, `autopilot_restore_integration` | After touching runtime, serve, daemon client | [ADR-0096](../adr/0096-unified-session-daemon.md) |
| Round/turn loop, built-in tools | `neenee-agent` inline tests; `orchestration`, `duplex`, `session_round_trip` | After touching the agent loop or tools | [Rounds and turns](../explanation/agent-design/rounds-and-turns.md) |
| TUI rendering | `neenee-tui` inline tests; `snapshot_tests` | After touching view tree, tool steps, diffs | [TUI component showcase](showcase.md) |
| Provider wire decoding | `neenee-providers` `tests/wire.rs` (mockito mock server) | After protocol parsing changes | [Provider capabilities](../explanation/provider-capabilities.md) |
| Persistence and paths | `neenee-persistence` inline tests; `tests/websearch_keys.rs` | After storage or `Dirs` changes | [Persistence](../explanation/persistence.md) |
| Wire contracts | `neenee-contracts` inline tests; `tests/tokenizer_corpus.rs` | After wire/tokenizer changes; regenerates the panel's `wire.gen.ts` | [Crate layering](../explanation/crate-layering.md) |
...
|---------|---------------|
| Tests fail only when an installed daemon is running, or a suite hangs on connect | Isolation leak: a test reached the host daemon. Confirm the resolved instance with `daemon status --diagnostic`; re-export `NEENEE_HOME` |
| `client/daemon binary mismatch` after a rebuild | Expected dev-drift gate, not a test bug — `neenee daemon stop`, rerun; the daemon respawns on demand |
| Port 9800 in use, or a daemon landed on an ephemeral port | Two instances contend for the default port. Give the dev instance `NEENEE_PORT` (CI uses 9800 for e2e only) |
| A TUI snapshot fails with a `.snap.new` file | Real rendering change. Review the `.snap.new`; when the new frame is correct, accept it with `INSTA_UPDATE=always cargo test -p neenee-tui <filter>` and commit the updated `.snap` |
| Files appear under the real `~/.cache/neenee` or `$XDG_CACHE_HOME` after a run | A test wrote outside its sandbox. The `test-path-override` hook exists for this; see [Add a test](#add-a-test) |
| `cargo test --workspace` fails only on the second run | Leftover state from a prior run in the instance root; wipe `NEENEE_HOME` and rerun |
...
RUSTFLAGS="-Zsanitizer=address" \
  cargo +nightly test -p neenee-tui-engine --target x86_64-unknown-linux-gnu
```
...
- Sandbox every test that touches the filesystem or the daemon: either
  ...
  `neenee-persistence`, which exposes `paths::set_test_default`. The
...
- New TUI renderings get an `insta` snapshot test in
  ...
  `INSTA_UPDATE=always cargo test -p neenee-tui <filter>` — this
...
The TUI component playground (`neenee showcase <component>`) renders
...
- [Dev and test isolation](dev-and-test-isolation.md) — why `NEENEE_HOME`/`--home` is mandatory and how the daemon sandbox is inherited
