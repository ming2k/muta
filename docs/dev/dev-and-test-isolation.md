...
neenee — its daemon, its sessions, its config, its credentials, its logs, or
...
`~/.config/neenee` and `~/.local/share/neenee`, and the same TCP port 9800.
Without isolation:
...
`--home <dir>` (the flag) and `NEENEE_HOME=<dir>` (the environment
...
`NEENEE_PORT=<port>` takes a default TCP port that cannot contend with the
...
important way: at startup it is restated as `NEENEE_HOME` in the process
...
```bash
export NEENEE_HOME=/tmp/neenee-dev
export NEENEE_PORT=9801
./target/debug/neenee daemon status --diagnostic   # confirms the instance
./target/debug/neenee attach
```
...
```bash
./target/debug/neenee --home /tmp/neenee-dev daemon stop
rm -rf /tmp/neenee-dev
```
...
The `test` job sets a process-wide `NEENEE_HOME` under `runner.temp`, and
the daemon-smoke e2e job starts its daemon under `NEENEE_HOME` +
`NEENEE_PORT` — one variable instead of the four `XDG_*` exports it used to
...
- `client::spawn_daemon` must keep inheriting the environment (no
  ...
  `NEENEE_HOME` at startup: sandbox inheritance for spawned daemons depends
  on both.
  `lifecycle_integration::spawned_daemon_inherits_the_neenee_home_sandbox`
