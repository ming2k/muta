...
use crate::tui::model::document::{Block, TranscriptMessage};
use crate::tui::model::layout::{LayoutMap, SemanticCursor, TableCellSegment};
use std::borrow::Cow;

/// Re-exported grapheme-boundary primitives so the rest of the crate keeps the
/// existing `crate::tui::model::selection::…` import path. The implementations live in
/// the engine ([`neenee_tui_engine::text`]), the single owner of grapheme/width
/// measurement.
pub use neenee_tui_engine::text::{floor_grapheme_boundary, inclusive_grapheme_end};
...
mod tests {
    use super::*;
    use crate::tui::model::document::{Block, Inline, TranscriptMessage};
    ...
    fn table_block_copy_strips_borders() {
        ...
        msg.blocks = vec![Block::Table {
            ...
            aligns: vec![crate::tui::model::document::TableAlignment::None; 2],
            rendered: String::new(),
        }];
        let messages = vec![msg];
        let sel = SelectionState::Block {
    ...
    fn table_cell_copy_includes_wrapped_text() {
        ...
        msg.blocks = vec![Block::Table {
            ...
            aligns: vec![crate::tui::model::document::TableAlignment::None],
            rendered: String::new(),
        }];
        let messages = vec![msg];
