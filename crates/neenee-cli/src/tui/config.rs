...
//!
//! The serialisable data struct ([`TuiConfig`]) lives in `neenee-persistence::config`
...
//! built-in presenter default. That lookup touches `crate::tui::view::tools`,
...
pub fn thinking_default_expanded(config: &TuiConfig) -> bool {
    config
        .default_expanded
        .get(neenee_persistence::config::THINKING_KEY)
        .copied()
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn thinking_defaults_collapsed_and_is_overridable() {
        ...
        let cfg = config(&[(neenee_persistence::config::THINKING_KEY, true)]);
