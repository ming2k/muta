...
/// (`neenee resume` with no id). In that mode Esc/click-outside quits the
...
pub fn draw_sessions_modal(
    ...
    session_detail: Option<&neenee_core::SessionDetail>,
    ...
    let para = neenee_tui_engine::Paragraph::new(body);
...
fn detail_body(detail: &neenee_core::SessionDetail, theme: &Theme) -> Vec<Line<'static>> {
