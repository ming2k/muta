...
use crate::tui::tools::{ArgLayout, DiffCache, DiffHunk, DiffOp, ResultKind, ToolStatus};
use crate::tui::view::{
...
fn draw_tool_result(
    ...
    let kind = crate::tui::tools::presenter_for(name).result_kind();
    match kind {
        ResultKind::Listing => {
...
fn bash_command_for(structured: Option<&neenee_core::ToolOutput>, arguments: &str) -> String {
    ...
    crate::tui::model::document::parse_arguments_kv(arguments)
        .iter()
        .find(|(k, _)| k == "command")
        .map(|(_, v)| v.clone())
        .unwrap_or_default()
...
pub fn draw_tool_step(
    ...
    if expanded {
...

        {
            ...
            if let crate::tui::model::document::MessageKind::ToolStep {
                name,
                arguments,
                output,
                structured,
            ...
            } = &msg.kind
            {
                ...
                if matches!(
                    crate::tui::tools::presenter_for(name).arg_layout(),
                    ArgLayout::KeyValue
                ) {
                    let kv = crate::tui::model::document::parse_arguments_kv(arguments);
                    if !kv.is_empty() {
        ...
        if let crate::tui::model::document::MessageKind::ToolStep { children, .. } = &msg.kind {
