...
/// Adapter implementing [`neenee_transport::UiBridge`] by delegating to the TUI's
...
impl neenee_transport::UiBridge for TuiClipboard {
    async fn copy_to_clipboard(&self, text: &str) -> Result<neenee_transport::CopyOutcome, String> {
        copy(text).await.map(|outcome| match outcome {
            CopyOutcome::Native => neenee_transport::CopyOutcome::Native,
            CopyOutcome::Osc52 => neenee_transport::CopyOutcome::Osc52,
        })
    }
}
