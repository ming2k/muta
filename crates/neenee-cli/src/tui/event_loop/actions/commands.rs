...
use neenee_core::{AgentRequest, AgentResponse, Role};

use crate::tui::clipboard;
use crate::tui::clipboard_ops;
use crate::tui::composer_attachments;
use crate::tui::model::document::{TranscriptMessage, UserMessageOrigin};
...
pub(super) async fn handle_send_slash(
    ...
    if cmd == "/serve" || cmd.starts_with("/serve ") {
        ...
        if tap.is_some() {
            ...
            if cmd == "/serve" {
                *tap = None;
                ...
                runtime.messages.write().await.push(
                    TranscriptMessage::new(Role::Assistant, "Serve mode stopped.".to_string())
                        .with_origin(UserMessageOrigin::Slash),
                );
            } else {
                runtime.messages.write().await.push(
                    TranscriptMessage::new(
                        Role::Assistant,
                        "Serve already active. Use /serve (no args) to stop.".to_string(),
                    )
                    .with_origin(UserMessageOrigin::Slash),
                );
            }
            ...
            *tap = Some(bc_tx.clone());
            ...
            let registry = Arc::new(neenee_transport::registry::SessionRegistry::prehost_only());
            registry
                .host(neenee_transport::registry::HostedSession {
                    ...
                    // A `/serve` prehost does not own the
                    // session lifecycle — this TUI does, and
                    // fires SessionEnd itself on exit.
            ...
            runtime.messages.write().await.push(
                TranscriptMessage::new(Role::Assistant, msg).with_origin(UserMessageOrigin::Slash),
            );
        }
        ...
        // The command is fully handled above (listener started,
        // stopped, or a notice painted) — return to the event loop.
        // This used to exit the whole TUI; that was a bug.
        return ActionFlow::NextEvent;
    }
