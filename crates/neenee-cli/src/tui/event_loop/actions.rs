...
use neenee_core::{AgentRequest, PermissionDecision, PermissionRequest};

use crate::tui::clipboard;
use crate::tui::clipboard_ops;
use crate::tui::input;
use crate::tui::model::document::NoticeSeverity;
use crate::tui::model::layout::InteractiveTargetKind;
use crate::tui::model::selection::SelectionState;
use crate::tui::view;
use crate::tui::view::Theme;
use crate::tui::{ActivityTab, App, Modal};
...
pub(super) async fn dispatch_action(
    ...
    match action {
        ...
        input::InputAction::SelectProviderTemplate => {
            if app.active_modal == Modal::ProviderTemplate
                && let Some(template) = crate::tui::PROVIDER_TEMPLATES.get(app.template_choice)
            {
        ...
        input::InputAction::ConfigThemeField { delta } => {
            ...
            {
                app.theme = Theme::from_color_scheme("custom", &app.custom_color_draft);
                let count = crate::tui::view::overlays::config_theme_custom::ROW_COUNT as i32;
        ...
        input::InputAction::ConfigLayoutApply => {
            ...
            {
                app.transcript_layout = crate::tui::view::layout::Strategy::from_config(value);
                let _ = app
                    .tx
        ...
        input::InputAction::QuestionUp => {
            ...
            {
                app.question = Some(qm.update(crate::tui::question_model::QuestionAction::Up).0);
        ...
        input::InputAction::QuestionSubmit => {
            if app.active_modal == Modal::Question
                && let Some(qm) = app.question.take()
            {
                let (qm, effects) = qm.update(crate::tui::question_model::QuestionAction::Submit);
        ...
        input::InputAction::QuestionCancel => {
            if app.active_modal == Modal::Question
                && let Some(qm) = app.question.take()
            {
                let (_qm, effects) = qm.update(crate::tui::question_model::QuestionAction::Cancel);
