...
use crate::tui::view;
use crate::tui::{App, Modal, ProviderDeleteChoice, Recess};
...
pub(super) fn render_frame(
    ...
    if !chrome_hidden && let Some(epoch) = app.effort_ignition_epoch {
        ...
        crate::tui::effort_ignition::paint_ignition_bands(f, input_rect, hint_row, elapsed_ms);
    }
        ...
        Modal::TokenReport => {
            ...
            Some(view::draw_token_report_modal(
                ...
                view::ContextUsageView {
                    ...
                    window_tokens: crate::tui::providers::model_context_window(&app.current_model),
            ...
            let user_prompt: Option<String> = app
                ...
                .find(|m| {
                    m.role == neenee_core::Role::User
                        && m.origin == crate::tui::model::document::UserMessageOrigin::Chat
                })
                .map(|m| m.raw.clone());
