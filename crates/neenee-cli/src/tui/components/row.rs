...
//!
//! ```ignore
//! use crate::tui::components::row::{ListRow, RowGroup};
...
use neenee_tui_engine::{Color, Line, Span, Style};
    ...
    for (atom, gap) in atoms {
        ...
        let style = match atom {
            GroupAtom::Plain { fg, bold, .. } => {
                ...
                if *bold {
                    s = s.add_modifier(neenee_tui_engine::Modifier::BOLD);
                }
                s
            }
...
mod tests {
    ...
    fn line_text(line: &neenee_tui_engine::Line) -> String {
        line.spans
            .iter()
    ...
    fn status_glyphs_are_bold_icons_not_text() {
        ...
        assert!(
            dot_span
                .style
                .add
                .contains(neenee_tui_engine::Modifier::BOLD),
            "glyph atom is bold"
        );
    }
}
