    ...
    match action {
        SkillAction::List => {
            ...
            if result.skills.is_empty() {
                ...
                println!("Tip: Place skills in .neenee/skills/<name>/SKILL.md or ~/.local/share/neenee/skills/.");
                return Ok(());
            }

            println!("{:<20} {:<12} {:<10} Description", "Skill Name", "Scope", "Version");
            println!("{:-<20} {:-<12} {:-<10} {:-<30}", "", "", "", "");

            for skill in &result.skills {
                let scope = format!("{:?}", skill.scope);
                let version = skill.version.as_deref().unwrap_or("–");
                let desc = skill.short_description.as_deref().unwrap_or(skill.description.as_str());
                let short_desc = if desc.chars().count() > 50 {
                    format!("{}...", desc.chars().take(47).collect::<String>())
                } else {
                    desc.to_string()
                };
                println!("{:<20} {:<12} {:<10} {}", skill.name, scope, version, short_desc);
            }
