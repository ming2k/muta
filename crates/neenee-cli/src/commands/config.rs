...
pub fn run(action: ConfigAction) -> Result<(), Box<dyn std::error::Error>> {
    match action {
        ...
        ConfigAction::List => {
            ...
            println!("default_provider: {}", if config.default_provider.is_empty() { "(none)" } else { &config.default_provider });
            println!("default_model:    {}", config.default_model.as_deref().unwrap_or("(none)"));
            ...
            println!("compaction_preserve_rounds: {}", config.compaction_preserve_rounds);
            println!("compaction_summarize:       {}", config.compaction_summarize);
        ...
        ConfigAction::Set { key, value } => {
            ...
            match key.as_str() {
                ...
                "provider_retry_max_attempts" => {
                    config.provider_retry_max_attempts = value.parse().map_err(|_| "invalid integer for retry_max_attempts")?;
                }
                "provider_retry_base_ms" => {
                    config.provider_retry_base_ms = value.parse().map_err(|_| "invalid integer for retry_base_ms")?;
                }
                "provider_retry_max_ms" => {
                    config.provider_retry_max_ms = value.parse().map_err(|_| "invalid integer for retry_max_ms")?;
                }
                "compaction_preserve_rounds" => {
                    config.compaction_preserve_rounds = value.parse().map_err(|_| "invalid integer for compaction_preserve_rounds")?;
                }
                "compaction_summarize" => {
                    config.compaction_summarize = value.parse().map_err(|_| "invalid boolean (true/false) for compaction_summarize")?;
                }
                "compaction_prune" => {
                    config.compaction_prune = value.parse().map_err(|_| "invalid boolean (true/false) for compaction_prune")?;
                }
                other => {
                    return Err(format!("unsupported or read-only configuration key '{other}'").into());
                }
            }
            config.save()?;
