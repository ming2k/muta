...
Built-in tools for the neenee agent stack: filesystem, shell, web, ask-user,
and todo management.
...
web-search backends in `search`. `neenee-agent` constructs the todo tools with
state owned by each agent instance; applications do not repeat that wiring.
