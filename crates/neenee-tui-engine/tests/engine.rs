...
use neenee_tui_engine::backend::{Backend, Bce};
use neenee_tui_engine::diff;
use neenee_tui_engine::{Cell, Color, Fit, Grid, Pos, Style};
...
fn bce_clear_to_end_uses_clr_eol_only_for_default_bg() {
    ...
    use neenee_tui_engine::diff::{Draw, DrawCmd};
    crossterm::style::force_color_output(true);
    let cmd = DrawCmd {
        w: 10,
        h: 10,
...
impl ModelTerminal {
    ...
    fn apply(&mut self, bytes: &str) {
        ...
        while let Some(c) = it.next() {
            ...
            let g = c.to_string();
            let width = neenee_tui_engine::text::grapheme_width(&g).max(1) as u16;
            if self.cx < self.w {
...
fn invalidate_emits_real_sgr_reset() {
    ...
    let bold_cmd = DrawCmd {
        ...
        draws: vec![Draw::Cells {
            x: 0,
            y: 0,
            style: Style::default().add_modifier(neenee_tui_engine::Modifier::BOLD),
