...
//! anywhere and silently fell back to a default. `neenee config check`
...
/// Kept for the drift test and any future `neenee config` completions: the
        ...
        if !known.contains(&key.as_str()) {
            out.push(ConfigFinding {
                ...
                message: if value.is_table() {
                    ...
                    "unknown key (ignored; check the spelling against `neenee config list`)"
                        .to_string()
                },
                is_legacy: false,
            });
