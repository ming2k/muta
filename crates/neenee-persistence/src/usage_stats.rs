...
use neenee_contracts::usage_stats::{
    UsageStatRecord, UsageStatsReport, aggregate_usage_records, day_key_from_epoch_ms,
};
use neenee_contracts::{RequestUsageKey, RequestUsageRecord, RequestUsageSource};
...
/// [`neenee_contracts::UsageStatSink`] adapter over the store, safe to share
/// as an `Arc` into a [`neenee_contracts::TokenSourceLedger`].
///
...
impl neenee_contracts::UsageStatSink for UsageStatsStore {
...
mod tests {
    use super::*;
    use neenee_contracts::{RequestUsageKey, RequestUsageStatus};
    ...
    fn ledger_sink_end_to_end_persists_and_aggregates() {
        ...
        neenee_contracts::TokenSourceLedger::install_usage_sink(
            &ledger,
            store.clone() as Arc<dyn neenee_contracts::UsageStatSink>,
        );
