...
impl SessionRegistry {
    ...
    pub async fn resolve(&self, action: AttachAction) -> ResolveOutcome {
        match action {
            AttachAction::New => match self
                .assemble_hosted(crate::startup::StartupMode::Fresh)
                .await
            {
    ...
    async fn resolve_auto(&self) -> ResolveOutcome {
        ...
        match map.len() {
            0 => {
                    ...
                    .await
                {
                    Ok(b) => ResolveOutcome::Welcome(b),
                    Err(AssembleErr::NoHost) => {
                        ResolveOutcome::Error("no session is available on this host".into())
                    }
                    Err(AssembleErr::AssembleFailed(e)) => {
                        ResolveOutcome::Error(format!("could not start a session: {e}"))
                    }
                }
            }
            1 => {
    ...
    async fn resolve_id(&self, id: &str) -> ResolveOutcome {
        {
            let map = self.sessions.lock().await;
        ...
        if !session_exists_on_disk(&p.project_root, id).await {
            return ResolveOutcome::Error(format!("unknown session id '{id}'"));
        }
        match self
            .assemble_hosted(crate::startup::StartupMode::Resume(Some(id.to_string())))
            .await
        {
            Ok(b) => ResolveOutcome::Welcome(b),
            Err(AssembleErr::NoHost) => {
