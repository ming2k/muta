...
use neenee_agent::{Agent, EnvoyTool, RoundLifecycle};
use neenee_core::{
...
pub(crate) use neenee_transport::{agent_setup, hooks, pursuits, side, startup};
...
use neenee_transport::startup::{BuiltinCmd, StartupMode, init_tracing, parse_args};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if session.pursuit().await.is_none() {
        ...
        if let Some(pursuit) =
            neenee_persistence::legacy_pursuit::read_legacy_pursuit(&legacy_db, &thread_id)
        {
    ...
    let driver = neenee_transport::SessionDriver {
        req_rx,
        tx: resp_tx,
        ...
        extra_commands: Arc::new(neenee_transport::slash_handler::SlashCommandRegistry::new()),
    };
