...
//!
//! Pure functions that turn Gemini's JSON response shape into the harness's
//! domain types. Gemini reports usage under `usageMetadata`
...
pub fn message(response: &Value) -> Result<Message, String> {
    let candidates = response
        ...
        .ok_or_else(|| format!("Invalid Gemini response: {}", response))?;
...
/// events. Gemini may stream text, usage metadata, and whole `functionCall`
...
/// Parse one `streamGenerateContent` SSE payload into normalized provider
/// events plus any Gemini thought signatures attached to function-call parts.
...
/// Whether this part is a Gemini reasoning *summary* — the model's thinking
...
/// Gemini, a `404 NOT_FOUND` almost always means the upstream does not serve
