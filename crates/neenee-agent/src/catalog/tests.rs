...
use neenee_contracts::catalog::Transport;
use neenee_contracts::{Effort, RemoteModelEndpoint, ThinkingMode};
...
fn copilot_route_uses_remote_endpoint_metadata() {
    ...
    cache.remote_metadata.insert("copilot".to_string(), {
        ...
        m.insert(
            "gpt-5".to_string(),
            neenee_contracts::RemoteModelMetadata {
...
fn channel_model_info_effort_ladders_survive() {
    ...
    let gemini37 = neenee_contracts::catalog::Channel {
...
fn sync_fitted_model_registry_overlays_fitted_ids() {
    ...
    let resolved = neenee_contracts::model::resolve("kimi-for-coding");
