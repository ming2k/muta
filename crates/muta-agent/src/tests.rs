...
impl Provider for HintProvider {
    ...
    fn prompt_hints(&self) -> muta_contracts::ProviderPromptHints {
        muta_contracts::ProviderPromptHints {
...
impl Tool for WriteTestTool {
    ...
    fn scope_target(&self, _arguments: &str) -> muta_contracts::ScopeTarget {
        muta_contracts::ScopeTarget::Path(std::path::PathBuf::from("/tmp/test"))
    }
...
async fn interrupt_settles_in_flight_request_with_estimated_prompt() {
    ...
    impl Provider for PendingProvider {
        async fn chat(&self, _: muta_contracts::ModelRequest) -> Result<muta_contracts::ProviderCompletion, String> {
            unreachable!("streaming path should be used")
        }
        async fn stream_chat(
            &self,
...
async fn interrupted_stream_books_exact_not_cumulative_completion_tokens() {
    ...
    let expected = muta_contracts::tokenizer::Tokenizer::new().count(&streamed) as i64;
    assert_eq!(
...
impl Provider for GatedRunnerProvider {
    async fn chat(&self, _request: muta_contracts::ModelRequest) -> Result<muta_contracts::ProviderCompletion, String> {
        Ok(muta_contracts::ProviderCompletion::message(Message::new(Role::Assistant, "gated")))
    }
    async fn stream_chat(
        &self,
        _request: muta_contracts::ModelRequest,
...
impl Provider for ScriptedProvider {
    async fn chat(&self, request: muta_contracts::ModelRequest) -> Result<muta_contracts::ProviderCompletion, String> {
        ...
        Ok(muta_contracts::ProviderCompletion::message(Message::new(Role::Assistant, reply)))
    }
...
impl Tool for RecordingTool {
    ...
    fn scope_target(&self, arguments: &str) -> muta_contracts::ScopeTarget {
        if self.name == "execute_command" {
            ...
            muta_contracts::ScopeTarget::Command(command)
            ...
            muta_contracts::ScopeTarget::Path(std::path::PathBuf::from(path))
        } else {
...
async fn command_policy_user_allow_overrides_builtin_confirm() {
    ...
    config
        .rules
        .push(muta_persistence::config::BashPolicyRuleConfig {
            name: "fixture allows reset".to_string(),
            matcher: muta_persistence::config::BashPolicyMatcherConfig::Contains,
            pattern: "git reset --hard".to_string(),
            action: muta_persistence::config::BashPolicyActionConfig::Allow,
            reason: None,
        });
