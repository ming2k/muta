...
use muta_contracts::{ChannelAuth, ClientIdentity, RemoteModelMetadata, SecretString};
...
use muta_providers::provider_preset_spec;
...
pub fn migrate_legacy_state() -> bool {
    let connections_path = muta_persistence::paths::get().connections_file();
    if connections_path.exists() && !Connections::load().connections.is_empty() {
        return false;
    }

    // Check if providers.toml exists from previous schema
    let old_providers_path = muta_persistence::paths::get()
        .state_dir
        .join("providers.toml");
    if old_providers_path.exists()
        && let Ok(content) = std::fs::read_to_string(&old_providers_path)
    {
        #[derive(Deserialize)]
...
fn override_base_url(
    ...
    if let Some(pid) = preset_id {
        let preset_default =
            muta_providers::route_for_model(pid, legacy.models.first()?).map(|(_, base, _)| base);
...
impl LegacyConfig {
    fn read() -> Option<Self> {
        let path = muta_persistence::paths::get().config_file();
...
impl LegacyCredentials {
    fn read() -> Option<Self> {
        let path = muta_persistence::paths::get().credentials_file();
        let content = std::fs::read_to_string(&path).ok()?;
        toml::from_str(&content).ok()
    }
