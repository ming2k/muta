...
struct ReadArgs {
    #[tool(desc = "Path to the text file; relative paths use the primary workspace")]
    path: String,
...
impl Tool for ReadTextTool {
    ...
    fn parameters(&self) -> serde_json::Value {
        ...
        // JSON to drift. See `ReadArgs` / `muta_tool_derive::ToolSchema`.
    ...
    async fn call_structured(&self, arguments: &str) -> Result<muta_contracts::ToolOutput, String> {
        ...
        let start = offset - 1;
        if total_lines == 0 {
            return Ok(muta_contracts::ToolOutput::Code {
                lang,
                text: String::new(),
                start_line: offset,
        ...
        Ok(muta_contracts::ToolOutput::Code {
            lang,
            text,
            start_line: offset,
            prefix,
...
muta_contracts::register_tool!(ReadTextFactory => |ctx| ReadTextTool {
...
/// tight contract better than verbose usage guidance. An subagent on such a
...
muta_contracts::register_tool!(ReadTextTerseFactory => |ctx| ReadTextTerseTool {
    root: workspace_base(ctx),
    env: Some(execution_environment(ctx)),
});
