...
impl Tool for BashTool {
    ...
    async fn call_structured_with_events<'a>(
        ...
        let terminal_id = args["terminal_id"].as_str().or_else(|| {
            if args["run_persistent"].as_bool() == Some(true) {
                Some("default")
            } else {
                None
            }
        ...
        if let Some(term_id) = terminal_id {
            ...
            return session
                .run_command(command, timeout_duration, on_stream)
                .await;
        }
        ...
        let (mut child, process_tree) = {
            // Isolate the child from muta's controlling terminal. muta's
...
mod tests {
    ...
    /// child opening `/dev/tty` cannot reach muta's controlling terminal.
    ...
    async fn test_persistent_terminal_preserves_state() {
        ...
        // Step 2: Read that variable in a second command in the same persistent session
        let res2 = tool
...
impl PersistentTerminalSession {
    pub fn spawn(root: &std::path::Path) -> Result<Self, String> {
...
static PERSISTENT_TERMINALS: std::sync::LazyLock<
    tokio::sync::Mutex<
        std::collections::HashMap<
            String,
            std::sync::Arc<tokio::sync::Mutex<PersistentTerminalSession>>,
        >,
    >,
