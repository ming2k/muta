...
pub struct WebFetchTool {
    ...
    /// toolset.
    config: muta_contracts::SharedWebSearchConfig,
...
impl WebFetchTool {
    ...
    pub fn with_config(config: WebSearchConfig) -> Self {
        Self::with_shared_config(muta_contracts::SharedWebSearchConfig::new(config))
    }
    /// Share a hot-reloadable config handle instead of snapshotting a value.
    pub fn with_shared_config(config: muta_contracts::SharedWebSearchConfig) -> Self {
        Self {
            config,
...
/// `muta/0.1 (+ai-coding-agent)` UA was rejected by anti-bot layers far more
