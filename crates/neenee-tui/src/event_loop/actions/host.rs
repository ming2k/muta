...
fn verb_request(verb: ConsoleVerb, id: String) -> neenee_runtime::serve::ControlRequest {
    match verb {
        ConsoleVerb::Kill => neenee_runtime::serve::ControlRequest::KillSession { session_id: id },
        ConsoleVerb::Interrupt => {
            neenee_runtime::serve::ControlRequest::Interrupt { session_id: id }
        }
        ConsoleVerb::Suspend => {
            neenee_runtime::serve::ControlRequest::SuspendSession { session_id: id }
        }
    }
}
...
async fn discover_and_control(
    request: neenee_runtime::serve::ControlRequest,
) -> Result<(), String> {
    let project_root = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
    match neenee_runtime::client::discover(&project_root) {
        Some(info) => neenee_runtime::client::control(&info, request).await,
        None => {
...
async fn dispatch_create(app: &mut App, runtime: &UiRuntime, raw: &str, text: Option<String>) {
    ...
    tokio::spawn(async move {
        let outcome = discover_and_control(neenee_runtime::serve::ControlRequest::CreateSession {
            project,
            prompt: text,
        })
        .await;
...
pub(super) async fn dispatch_console_command(
    ...
    match crate::overlays::parse_console_command(raw) {
        ...
        ConsoleCommand::Prompt { targets, text } => {
            ...
            let sends: Vec<(usize, neenee_runtime::serve::ControlRequest)> = resolved
                .into_iter()
                .map(|(n, id)| {
                    (
                        n,
            ...
            tokio::spawn(async move {
                ...
                for (n, request) in sends {
                    let line = match neenee_runtime::client::control(&info, request).await {
                        Ok(()) => ConsoleLine::Receipt {
                            ok: true,
                            target: Some(n),
