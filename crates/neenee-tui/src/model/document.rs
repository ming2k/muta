...
pub enum MessageKind {
    ...
    ToolStep {
        ...
        /// [`neenee_contracts::ToolOutput`] carrying structured data (e.g. a shell
        ...
        structured: Option<Box<neenee_contracts::ToolOutput>>,
...
pub fn notice_severity_from_core(severity: neenee_contracts::NoticeSeverity) -> NoticeSeverity {
    match severity {
        neenee_contracts::NoticeSeverity::Info => NoticeSeverity::Info,
        neenee_contracts::NoticeSeverity::Warning => NoticeSeverity::Warning,
        neenee_contracts::NoticeSeverity::Error => NoticeSeverity::Error,
    }
}
...
impl TranscriptMessage {
    ...
    /// chunk initializes a partial [`neenee_contracts::ToolOutput::Shell`]; the
    /// terminal `finish_tool_step` later overwrites it with the final result.
    /// Returns `false` if this isn't a matching running step.
    pub fn push_tool_stream(&mut self, id: &str, stream: &neenee_contracts::ToolStream) -> bool {
        let MessageKind::ToolStep {
            id: step_id,
            structured,
            status,
        ...
        if !matches!(
            structured.as_deref(),
            Some(neenee_contracts::ToolOutput::Shell { .. })
        ) {
            *structured = Some(Box::new(neenee_contracts::ToolOutput::Shell {
                command: String::new(),
                stdout: String::new(),
                stderr: String::new(),
                lines: Vec::new(),
                ...
                termination: neenee_contracts::tool_output::ShellTermination::default(),
            }));
        }
        if let Some(neenee_contracts::ToolOutput::Shell {
            stdout,
            stderr,
            lines,
            ..
        }) = structured.as_deref_mut()
        {
            ...
            let stream_tag = match stream {
                neenee_contracts::ToolStream::Stdout(_) => {
                    neenee_contracts::tool_output::ShellStream::Out
                }
                neenee_contracts::ToolStream::Stderr(_) => {
                    neenee_contracts::tool_output::ShellStream::Err
                }
            };
            let text = match stream {
                neenee_contracts::ToolStream::Stdout(s)
                | neenee_contracts::ToolStream::Stderr(s) => s,
            };
            for piece in text.split('\n') {
                if !piece.is_empty() {
                    lines.push(neenee_contracts::tool_output::ShellLine {
                        stream: stream_tag,
            ...
            match stream {
                neenee_contracts::ToolStream::Stdout(s) => stdout.push_str(s),
                neenee_contracts::ToolStream::Stderr(s) => stderr.push_str(s),
            }
        }
        self.refresh_tool_step();
        true
            ...
            EnvoyEvent::ToolResult {
                ...
                if let Some(child) = children.iter_mut().find(|m| {
                    ...
                    child.finish_tool_step(
                        id,
                        output.clone(),
                        neenee_contracts::ToolOutput::text(output.clone()),
                        *duration_ms,
                    );
                } else {
    ...
    pub fn envoy_outcome_line(&self) -> Option<String> {
        ...
        let source: &str = match structured.as_deref() {
            Some(neenee_contracts::ToolOutput::Envoy { summary, .. }) => summary,
            _ => output.as_deref()?,
        };
        source
            .lines()
...
mod tests {
    ...
    fn tool_step_collapses_and_restores_full_semantic_detail() {
        ...
        assert!(message.finish_tool_step(
            "call_1",
            "contents",
            neenee_contracts::ToolOutput::text("contents"),
            1234
        ));
    ...
    fn envoy_started_event_labels_step_by_role() {
        ...
        assert_eq!(task.envoy_role(), None);
        assert!(
            task.push_envoy_event(&neenee_contracts::EnvoyEvent::Started {
    ...
    fn envoy_status_reflects_children_and_completion() {
        ...
        assert!(task.finish_tool_step(
            "call_9",
            "final answer",
            neenee_contracts::ToolOutput::text("final answer"),
            1500
        ));
        assert!(
    ...
    fn envoy_failed_status_reports_failure() {
        ...
        let structured = neenee_contracts::ToolOutput::Envoy {
    ...
    fn envoy_peek_reports_awaiting_approval_while_parked() {
        ...
        task.push_envoy_event(&EnvoyEvent::PermissionRequest(
            neenee_contracts::PermissionRequest {
                id: "p1".into(),
                tool: "bash".into(),
                label: "Run rm".into(),
    ...
    fn bash_failure_is_classified_failed_from_structured_exit_code() {
        ...
        let structured = neenee_contracts::ToolOutput::Shell {
            command: "false".into(),
            stdout: String::new(),
            stderr: "boom".into(),
            lines: Vec::new(),
    ...
    fn push_tool_stream_builds_interleaved_lines_for_live_view() {
        ...
        use neenee_contracts::{ToolStream, tool_output::ShellStream};
        ...
        let lines = match &step.kind {
            MessageKind::ToolStep {
                ...
                neenee_contracts::ToolOutput::Shell { lines, .. } => lines,
        ...
        match step.kind {
            MessageKind::ToolStep {
                ...
                neenee_contracts::ToolOutput::Shell { stdout, stderr, .. } => {
    ...
    fn cancel_tool_step_transitions_to_a_terminal_state() {
        ...
        assert!(!step.finish_tool_step(
            "call_1",
            "late result",
            neenee_contracts::ToolOutput::text("late result"),
            10
        ));
        assert!(!step.cancel_tool_step("call_1"));
        assert_eq!(step.tool_step_status(), Some(ToolStepStatus::Cancelled));
