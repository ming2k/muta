        ...
        |f, s| {
            ...
            let render = draw_transcript(
                ...
                TranscriptView {
                    ...
                    carousel_index: 0,
                    theme: &theme,
                    layout: crate::view::layout::Strategy::default(),
                    height_cache: None,
                },
            );
...
fn chat_flow() -> Vec<TranscriptMessage> {
    ...
    let mut grep = TranscriptMessage::tool_step(
        "grep_bg",
        "grep",
        r#"{"pattern":"theme.surface","path":"crates/neenee-cli/src/tui/render"}"#,
    );
...
fn draw_title(f: &mut neenee_tui_engine::Frame, text: &str, theme: &Theme) {
    use neenee_tui_engine::{Line, Rect, Span, Style};

    let area = f.area();
    let row = Rect::new(area.x, area.y, area.width, 1);
    let line = Line::from(Span::styled(text, Style::default().fg(theme.muted())));
    f.render_widget(
        neenee_tui_engine::Paragraph::new(line).style(Style::default().bg(theme.surface())),
        row,
    );
}
