...
use neenee_contracts::{ProviderPickerRow, ProviderPickerSnapshot, SessionOverview};
use neenee_contracts::{TodoId, TodoItem, TodoList, TodoStatus};
...
pub fn models() -> io::Result<()> {
    ...
    let mk = |id: &str, name: &str, template_id: &str, models: &[&str]| ProviderPickerRow {
        ...
        model_info: models
            .iter()
            .map(|m| neenee_contracts::ProviderModelInfo {
                model: m.to_string(),
                protocol: String::new(),
                effort: None,
                thinking: None,
...
struct HistoryState {
    history: Vec<neenee_contracts::HistoryEntry>,
    query: String,
    cursor: usize,
    index: usize,
}
