...
pub fn draw_todo_bar(
    ...
    use neenee_contracts::{TodoItem, TodoStatus};
...
mod tests {
    ...
    fn todo_list_with(
        item: &str,
        status: neenee_contracts::TodoStatus,
    ) -> neenee_contracts::TodoList {
    ...
    fn todo_row_text(todos: &neenee_contracts::TodoList, width: u16) -> String {
        let mut terminal = neenee_tui_engine::TestTerminal::new(width, 1);
        terminal.draw(|frame| {
    ...
    fn todo_bar_leads_with_brand_tag_on_a_plain_surface() {
        ...
        let theme = Theme::default();
        let todos = todo_list_with("write the docs", neenee_contracts::TodoStatus::InProgress);
    ...
    fn todo_bar_falls_back_to_first_pending_when_nothing_is_in_progress() {
        let todos = todo_list_with("write the docs", neenee_contracts::TodoStatus::Pending);
        let text = todo_row_text(&todos, 80);
        assert!(text.contains("TODOS 0/1"), "row was {text:?}");
    ...
    fn todo_bar_keeps_real_gap_before_the_legend() {
        ...
        let todos = todo_list_with(
            ...
            neenee_contracts::TodoStatus::InProgress,
        );
    ...
    fn hint_bar_renders_model_and_context() {
        ...
        let messages = vec![TranscriptMessage::new(neenee_contracts::Role::User, "hi")];
        terminal.draw(|f| {
            draw_hint_bar(
                f,
                Rect::new(0, 2, 80, 1),
