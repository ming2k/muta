...
fn termination_footer(
    term: neenee_contracts::tool_output::ShellTermination,
    theme: &Theme,
) -> Option<(String, Style)> {
    use neenee_contracts::tool_output::ShellTermination as T;
...
fn draw_bash_content(
    ...
    block_idx: usize,
    content: &str,
    structured: Option<&neenee_contracts::ToolOutput>,
    command: &str,
    selection: &SelectionState,
    indent: usize,
    inner_w: usize,
...
fn emit_bash_lines(
    ...
    for logical_line in text.split('\n') {
        ...
        let logical_line = neenee_contracts::tool_output::normalize_carriage_returns(logical_line);
        let wrapped = nonempty_wrapped(wrap_text(&logical_line, wrap_w));
        for wl in &wrapped {
            let block_wl = WrappedLine {
                text: wl.text.clone(),
...
fn bash_structured_lines(
    lines: &[neenee_contracts::tool_output::ShellLine],
    stdout: &str,
    stderr: &str,
    base: Style,
    stderr_style: Style,
) -> Vec<(String, Style)> {
    use neenee_contracts::tool_output::ShellStream;
    ...
    match kind {
        ...
        ResultKind::Code => {
            ...
            let (content, start_line, lang) = match structured {
                Some(neenee_contracts::ToolOutput::Code {
                    text,
                    start_line,
                    lang,
                    ..
                }) => (text.as_str(), *start_line, lang.as_deref()),
                Some(neenee_contracts::ToolOutput::Patch {
                    new, start_line, ..
...
/// [`ToolOutput::Shell`](neenee_contracts::ToolOutput) payload (set as soon as the
...
/// structured payload.
fn bash_command_for(structured: Option<&neenee_contracts::ToolOutput>, arguments: &str) -> String {
    if let Some(neenee_contracts::ToolOutput::Shell { command, .. }) = structured
        && !command.is_empty()
    {
...

        {
            ...
            } = &msg.kind
            {
                ...
                let bash_streaming = matches!(
                    structured.as_deref(),
                    Some(neenee_contracts::ToolOutput::Shell { stdout, .. }) if !stdout.is_empty()
                );
