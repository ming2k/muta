...
//! [`LayoutMap`](crate::model::layout::LayoutMap) as a [`BlockRegion`] under
...
impl SelectableRow {
    ...
    pub(crate) fn from_line(line: neenee_tui_engine::Line<'static>) -> Self {
...
pub(crate) fn render_selectable_body(
    ...
    let seg_width = |s: &RowSegment| -> u16 { neenee_tui_engine::text::str_width(&s.text) as u16 };
        ...
        wrapped.iter().enumerate().skip(start)
    {
        ...
        frame.render_widget(neenee_tui_engine::Paragraph::new(line), rect);
...
mod tests {
    ...
    fn registers_one_region_per_visual_row() {
        ...
        let mut terminal = neenee_tui_engine::TestTerminal::new(20, 10);
    ...
    fn empty_row_occupies_one_visual_row() {
        ...
        let mut terminal = neenee_tui_engine::TestTerminal::new(20, 10);
        let rows = vec![
            SelectableRow::styled("one", neenee_tui_engine::Style::default()),
            SelectableRow::empty(),
            SelectableRow::styled("two", neenee_tui_engine::Style::default()),
        ];
        let mut scroll = 0;
        let mut map = LayoutMap::new();
        terminal.draw(|f| {
    ...
    fn selection_extract_spans_rows() {
        let theme = Theme::default();
        let mut terminal = neenee_tui_engine::TestTerminal::new(40, 10);
        let rows = vec![
            SelectableRow::styled("hello world", neenee_tui_engine::Style::default()),
            SelectableRow::styled("second row", neenee_tui_engine::Style::default()),
        ];
        let mut scroll = 0;
        let mut map = LayoutMap::new();
    ...
    fn multi_segment_row_round_trips() {
        ...
        let rows = vec![SelectableRow::from_segments(vec![
            RowSegment::styled("Total  ", neenee_tui_engine::Style::default()),
            RowSegment::styled("12345", neenee_tui_engine::Style::default()),
        ])];
        let mut scroll = 0;
        let mut map = LayoutMap::new();
        terminal.draw(|f| {
    ...
    fn drag_chain_from_press_to_copy() {
        ...
        let mut terminal = neenee_tui_engine::TestTerminal::new(40, 10);
        let rows = vec![
            SelectableRow::styled("alpha", neenee_tui_engine::Style::default()),
            SelectableRow::styled("beta", neenee_tui_engine::Style::default()),
            SelectableRow::styled("gamma", neenee_tui_engine::Style::default()),
        ];
        let mut scroll = 0;
        let mut map = LayoutMap::new();
        terminal.draw(|f| {
