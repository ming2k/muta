...
use neenee_tui_engine::{Frame, Modifier, Rect, Span, Style};
...
fn round_interrupt_header_line(
    warn: neenee_tui_engine::Color,
    time_label: Option<&str>,
    muted: neenee_tui_engine::Color,
    full_width: usize,
) -> neenee_tui_engine::Line<'static> {
    ...
    neenee_tui_engine::Line::from(spans)
}
    ...
    *content_lines += 1;
    if *skip_rows > 0 {
        *skip_rows -= 1;
        ...
        frame.render_widget(neenee_tui_engine::Paragraph::new(header_line), line_rect);
    ...
    for wl in body_lines {
        *content_lines += 1;
        ...
        frame.render_widget(
            neenee_tui_engine::Paragraph::new(neenee_tui_engine::Line::from(vec![Span::styled(
                wl.text,
...
mod tests {
    ...
    fn header_is_glyph_label_and_optional_time() {
        let line = round_interrupt_header_line(
            neenee_tui_engine::Color::Yellow,
            Some("21:39"),
            neenee_tui_engine::Color::DarkGray,
            40,
        );
