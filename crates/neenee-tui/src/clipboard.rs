...
/// Adapter implementing [`neenee_runtime::UiBridge`] by delegating to the TUI's
...
impl neenee_runtime::UiBridge for TuiClipboard {
    async fn copy_to_clipboard(&self, text: &str) -> Result<neenee_runtime::CopyOutcome, String> {
        copy(text).await.map(|outcome| match outcome {
            CopyOutcome::Native => neenee_runtime::CopyOutcome::Native,
            CopyOutcome::Osc52 => neenee_runtime::CopyOutcome::Osc52,
        })
    }
}
