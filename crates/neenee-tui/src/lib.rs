...
//!   model, painting `neenee_contracts` domain types into the engine grid:
...
//!   `neenee` binary (`neenee-cli`), which stays a thin shell over this
//!   crate. The debug-only [`showcase`] module is a "Storybook" rendering
//!   individual components in isolation (`neenee showcase <component>`).
//!
...
pub async fn run_tui(
    ...
    input_history: Vec<neenee_contracts::HistoryEntry>,
    ...
    token_ledger: Option<Arc<neenee_contracts::TokenSourceLedger>>,
    ...
    let pending_input = Arc::new(Mutex::new(VecDeque::<neenee_contracts::InputRequest>::new()));
    ...
    let session_detail = Arc::new(tokio::sync::Mutex::new(
        None::<neenee_contracts::SessionDetail>,
    ));
    ...
    let host_sessions = Arc::new(Mutex::new(Vec::<neenee_contracts::MonitoredSession>::new()));
    ...
    // a normal state and the panel simply shows an empty list.
    {
        ...
        tokio::spawn(async move {
            ...
            let action = neenee_contracts::MonitorAction {
                watch: true,
                include_idle: true,
            };
            let Ok(mut rx) = neenee_runtime::client::monitor_stream(&info, action).await else {
                return;
            };
            while let Some(event) = rx.recv().await {
                {
                    ...
                    match event {
                        neenee_contracts::MonitorEvent::Snapshot(snap) => {
                            *rows = snap.sessions;
                        }
                        neenee_contracts::MonitorEvent::SessionAdded(row)
                        | neenee_contracts::MonitorEvent::SessionUpdated(row) => {
                            neenee_runtime::client::upsert_session_row(&mut rows, row);
                        }
                        neenee_contracts::MonitorEvent::SessionRemoved { session_id } => {
                        ...
                        neenee_contracts::MonitorEvent::DaemonDraining => {}
                    }
                }
            ...
            match resp {
                ...
                AgentResponse::Round { session_id, event } => {
                    ...
                    match event {
                        ...
                        RoundEvent::Notice(notice) => {
                            ...
                            if notice.kind == neenee_contracts::NoticeKind::ProviderRetry {
                                // Skip the inline append; RetryScheduled owns
                                // the retry disclosure.
                            } else if notice.surface == neenee_contracts::NoticeSurface::Toast {
                        ...
                        RoundEvent::StreamReasoningDelta(delta) => {
                            ...
                            let hidden_chain = {
                                ...
                                !neenee_contracts::model_by_id(&model_id)
                        ...
                        RoundEvent::Envoy {
                            ...
                            match &event {
                                neenee_contracts::EnvoyEvent::PermissionRequest(req) => {
                                ...
                                neenee_contracts::EnvoyEvent::UserQuestionRequest(req) => {
                ...
                AgentResponse::ConnectStatus(status) => {
                    ...
                    match status {
                        ...
                        neenee_contracts::ConnectStatus::Done { provider } => {
                            *oauth_add_signal_clone.lock().await =
                        ...
                        neenee_contracts::ConnectStatus::DiscoveryWarning { provider, message } => {
                        ...
                        neenee_contracts::ConnectStatus::Failed { provider, message } => {
                            *oauth_add_signal_clone.lock().await =
    ...
    let mut app = App {
        ...
        custom_auth: neenee_contracts::ChannelAuth::ApiKey,
...
pub struct TuiOutcome {
    pub history: Vec<neenee_contracts::HistoryEntry>,
...
fn describe_todos_change(prev: Option<&TodoList>, new: Option<&TodoList>) -> Option<String> {
    ...
    let done = new.count(neenee_contracts::TodoStatus::Completed);
...
mod describe_todos_change_tests {
    ...
    use super::*;
    use neenee_contracts::{TodoId, TodoItem, TodoStatus};
