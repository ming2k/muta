...
use neenee_contracts::RequestUsageStatus;
use neenee_contracts::usage_stats::{UsageStatRecord, UsageStatsReport};
...
fn daily_chart(report: &UsageStatsReport, body_width: usize, theme: &Theme) -> Line<'static> {
    let newest: Vec<&neenee_contracts::usage_stats::UsageDayTotals> =
...
fn daily_table(report: &UsageStatsReport, body: &mut Vec<Line<'static>>, theme: &Theme) {
    let days: Vec<&neenee_contracts::usage_stats::UsageDayTotals> =
...
fn event_log(report: &UsageStatsReport, body: &mut Vec<Line<'static>>, theme: &Theme) {
    ...
    for event in &report.events {
        ...
        let tokens = if event.record.source == neenee_contracts::RequestUsageSource::Estimated {
            format!("~{tokens}")
        } else {
            tokens
        };
