...
impl AuthStore {
    ...
    pub fn load_from_paths(path: &std::path::Path, legacy_path: &std::path::Path) -> Self {
            ...
            && let Ok(store) = toml::from_str::<Self>(&content)
        {
            ...
            if let Ok(bytes) = toml::to_string_pretty(&store).map(|s| s.into_bytes()) {
                let _ = muta_persistence::fsutil::atomic_write_bytes(path, &bytes);
            }
    ...
    /// [`muta_persistence::fsutil::atomic_write_bytes`].
    pub fn save(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let bytes = toml::to_string_pretty(self)?.into_bytes();
        muta_persistence::fsutil::atomic_write_bytes(&Self::path(), &bytes)?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_legacy_auth_migration() {
        let unique_id = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let temp = std::env::temp_dir().join(format!("muta_test_auth_{unique_id}"));
