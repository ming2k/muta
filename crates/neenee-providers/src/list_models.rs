...
//! `neenee-persistence::config` and `neenee_agent::catalog::reconcile_provider_models`).
...
use neenee_contracts::{RemoteModelEndpoint, RemoteModelMetadata, SecretString, ThinkingSupport};
use serde_json::Value;

/// The protocol a discovery request speaks. Kept as a string (not the
/// `neenee_persistence::UserTransport` enum) so this module has zero dependency on
/// `neenee-persistence` — the calling layer maps its transport to one of these.
    ...
    pub fn remote_metadata(&self) -> RemoteModelMetadata {
        RemoteModelMetadata {
            ...
            effort_levels: self.effort_levels.as_ref().map(|levels| {
                ...
                levels
                    .iter()
                    .map(|level| neenee_contracts::EffortLevel::parse(level))
                    .collect()
            }),
        }
    }
...
fn anthropic_version() -> &'static str {
    neenee_llm_client::protocol::anthropic::request::ANTHROPIC_VERSION
}
...
mod tests {
    ...
    fn copilot_metadata_preserves_picker_and_endpoint_facts() {
        ...
        assert_eq!(
            ...
            Some(vec![
                neenee_contracts::EffortLevel::Known(neenee_contracts::Effort::Low),
                neenee_contracts::EffortLevel::Known(neenee_contracts::Effort::Medium),
                neenee_contracts::EffortLevel::Known(neenee_contracts::Effort::High)
            ])
        );
