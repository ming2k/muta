...
//! the exact freshly-built `muta` binary through `CARGO_BIN_EXE_muta`.
...
fn discovery_path(root: &Path) -> PathBuf {
    root.join("muta").join("instance").join("daemon.json")
}
...
async fn spawned_daemon_inherits_the_muta_home_sandbox() {
    ...
    command
        ...
        .stderr(Stdio::null())
        .current_dir("/");
    muta_runtime::client::configure_daemon_detachment(&mut command);
    ...
    let record = loop {
        if let Ok(bytes) = std::fs::read(&own_record)
            && let Ok(record) =
                serde_json::from_slice::<muta_runtime::serve_discovery::Discovery>(&bytes)
        {
            break record;
        }
        assert!(
