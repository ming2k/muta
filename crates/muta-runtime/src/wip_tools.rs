...
//!
//! These live in `muta-runtime` (not `muta-agent`) because they drive
...
pub type CheckWipFuture = std::pin::Pin<
    Box<
        dyn std::future::Future<
                Output = (Vec<muta_contracts::WipConflict>, muta_contracts::WipAdvice),
            > + Send,
    >,
>;
...
impl Tool for CheckWipTool {
    ...
    async fn call(&self, arguments: &str) -> Result<String, String> {
        ...
        match advice {
            muta_contracts::WipAdvice::ProceedScoped => out.push_str(
            ...
            muta_contracts::WipAdvice::Defer => out.push_str(
...
mod tests {
    ...
    async fn check_wip_surfaces_conflicts_and_advice() {
        ...
        let query: CheckWipQuery = Arc::new(|_paths, _concern| {
            Box::pin(async {
                (
                    vec![muta_contracts::WipConflict {
    ...
    async fn check_wip_clean_verdict_reads_as_proceed() {
        let query: CheckWipQuery =
            Arc::new(|_p, _c| Box::pin(async { (Vec::new(), muta_contracts::WipAdvice::Proceed) }));
        let tool = CheckWipTool::new(query);
