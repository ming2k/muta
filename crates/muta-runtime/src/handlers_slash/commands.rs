...
pub(crate) async fn role(mut env: SlashEnv<'_>, name: &str, args: &str, parts: &[&str]) {
    ...
    match target {
        ...
        Some(role_id) => {
            ...
            let target_workspace = if builtin == Some(muta_contracts::MainAgentRole::Developer) {
                let ws_arg = parts
                    .get(2..)
                    .map(|p| p.join(" "))
            ...
            if jobs.is_empty() {
                ...
                for j in jobs {
                    ...
                    let status_str = match &j.state {
                        ...
                        muta_contracts::JobState::Ready { .. } => "Ready (service)".to_string(),
    ...
    let Some(project_root_for_side) = project_root_for_side else {
        match route {
            TrustRoute::GrantAll | TrustRoute::Grant(TrustDomain::UserAssets) => {
                ...
                let mut snap = muta_contracts::WorkspaceSecuritySnapshot::new("workspace-free");
