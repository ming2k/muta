...
//! type v1 ships; the [`Hook`] trait (in `muta_contracts`) is shaped so `http`
//! and `mcp_tool` handlers can be added later without touching the loop.

use std::path::Path;
use std::time::Duration;

use muta_agent::async_trait;
use muta_agent::{Hook, HookContext, HookEvent, HookEventKind, HookOutcome};
use muta_persistence::config::HookSpec;
use serde_json::json;
    ...
    match &ctx.event {
        HookEvent::SessionStart { source } => {
            value["source"] = json!(match source {
                muta_agent::SessionSource::Startup => "startup",
                muta_agent::SessionSource::Resume => "resume",
            });
        }
