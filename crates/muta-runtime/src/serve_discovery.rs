...
impl Discovery {
    pub fn effective_local_endpoint(&self) -> Option<muta_platform::ipc::LocalEndpoint> {
        self.local_endpoint.clone().or_else(|| {
            self.uds_path
                .clone()
                .map(muta_platform::ipc::LocalEndpoint::UnixSocket)
        })
    }
}
...
pub fn default_local_endpoint() -> Result<muta_platform::ipc::LocalEndpoint, String> {
    let instance_dir = paths::get().instance_dir();
    let instance_key = format!("daemon-{}", paths::project_bucket_name(&instance_dir));
    muta_platform::ipc::endpoint_for_instance(instance_dir.join("daemon.sock"), &instance_key)
...
mod lease_tests {
    ...
    fn drop_removes_the_record_and_double_release_is_a_noop() {
        ...
        muta_persistence::fsutil::atomic_write_json(
            &path,
            &Discovery {
                pid: 1,
...
fn write_to(path: &Path, record: &Discovery) -> Result<(), String> {
    muta_persistence::fsutil::atomic_write_json(path, record)
}
