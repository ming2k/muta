...
(`neenee-agent`) and the frontends (the `neenee` TUI and the `apps/web`
browser panel today).
...
long-lived hosted sessions, multiplexes requests/responses, and exposes a
stable transport so any frontend can attach. Since ADR-0096 the daemon owns
every session: a `SessionRegistry` hosts N sessions across N projects, and
frontends are pure clients.

It depends on `neenee-agent` for the orchestration loop and on
`neenee-persistence` for durable state; frontends depend only on the surface
exposed here.
...
- **`bootstrap::assemble`** — lazy per-session assembly: the daemon pays the
  setup cost once per session, not once per process.
- **`registry`** — `SessionRegistry`: create / get / list / close hosted
...
- **`slash_handler`** — a `SlashCommandHandler` trait + `SlashCommandRegistry`
  ...
  (ADR-0054).
- **`serve`** — the hot-attach WebSocket transport: `Select{Monitor}` for the
  observability stream (ADR-0093), `Select{attach}` for session clients,
  `Select{control}` for management verbs (ADR-0096), graceful drain on
  shutdown (ADR-0101). Loopback by default; `--public` binds all interfaces
  and requires a bearer token (ADR-0054).
...
[crate layering](../../docs/explanation/crate-layering.md). See ADR-0037,
ADR-0054, and ADR-0096 for the design decisions.
