...
//!
//! These live in `neenee-runtime` (not `neenee-agent`) because they drive
...
impl Tool for CheckWipTool {
    ...
    async fn call(&self, arguments: &str) -> Result<String, String> {
        ...
        match advice {
            neenee_contracts::WipAdvice::ProceedScoped => out.push_str(
            ...
            neenee_contracts::WipAdvice::Defer => out.push_str(
...
mod tests {
    ...
    async fn check_wip_clean_verdict_reads_as_proceed() {
        let query: CheckWipQuery = Arc::new(|_p, _c| {
            Box::pin(async { (Vec::new(), neenee_contracts::WipAdvice::Proceed) })
        });
        let tool = CheckWipTool::new(query);
