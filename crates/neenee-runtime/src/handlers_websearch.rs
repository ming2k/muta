...
//!   hot-reload handle ([`neenee_contracts::SharedWebSearchConfig`]) so the
...
pub async fn update(
    config: &mut Config,
    shared: &Arc<neenee_contracts::SharedWebSearchConfig>,
    ...
    let mut creds = neenee_persistence::config::Credentials::load();
...
pub fn apply_reloaded(shared: &Arc<neenee_contracts::SharedWebSearchConfig>, reloaded: &Config) {
...
mod tests {
    ...
    async fn update_persists_behaviour_fields_and_hot_applies() {
        ...
        let shared = Arc::new(neenee_contracts::SharedWebSearchConfig::new(
        ...
        let creds = neenee_persistence::config::Credentials::load();
        assert_eq!(
            creds
                .websearch
                .tavily_api_key
    ...
    async fn update_rejects_unknown_backend_without_touching_state() {
        ...
        let shared = Arc::new(neenee_contracts::SharedWebSearchConfig::new(before.clone()));
        let (resp_tx, mut resp_rx) = tokio::sync::mpsc::unbounded_channel();
