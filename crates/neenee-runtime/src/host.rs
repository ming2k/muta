...
use neenee_persistence::lock::ProcessLock;
use std::sync::Arc;
...
async fn run_inner(
            ...
            match wait_for_lock(&lock_path, budget).await {
                ...
                Err(_) => {
                    return RunOutcome::StartupFailed(format!(
                        "another neenee daemon is running (lock held at {})",
                        lock_path.display()
                    ));
                }
            }
    ...
    tracing::info!(%reason, remaining_budget =? lifecycle.shutdown_grace, "neenee serve: draining");
    ...
    if !gate.forced() {
        ...
        for (name, why) in &hung {
            tracing::warn!(task = %name, %why, "neenee serve: task did not stop within the grace budget");
        }
    }
    ...
    if forced {
        tracing::warn!(
            %reason,
            "neenee serve: forced exit — some teardown work was abandoned (see the task warnings above)"
        );
