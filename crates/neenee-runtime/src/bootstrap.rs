...
pub async fn assemble(params: BootstrapParams) -> Result<Bootstrap, Box<dyn std::error::Error>> {
    ...
    // `neenee_contracts::tool_registry`. Stateful/meta tools that genuinely depend on the
    ...
    let base_tools: Arc<Vec<Arc<dyn neenee_contracts::Tool>>> = Arc::new(toolset.default_view());
