    ...
    let body = request::body(
        vec![
            ...
            Message {
                role: Role::Assistant,
                content: String::new(),
                tool_calls: Some(vec![muta_contracts::ToolCall {
...
fn request_body_includes_tools_in_anthropic_shape() {
    ...
    let request =
        muta_contracts::ModelRequest::with_tools(vec![Message::new(Role::User, "hi")], &tools);
...
fn cache_breakpoints_never_exceed_four_cap() {
    ...
    let history: Vec<Message> = (0..8)
        ...
        .collect();
    let request = muta_contracts::ModelRequest::with_tools(history, &tools);
    let (messages, tool_specs) = request.into_parts();
    let body = request::body(
        messages,
        BodyInput {
