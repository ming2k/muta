...
//! live on [`crate::config::OAuthConfig`]; these functions are the generic
...
pub(crate) async fn post_form(
    ...
    let response = req
        .send()
        .await
        .map_err(|e| crate::AuthError::Transport(format!("token request failed: {e}")))?;
    ...
    serde_json::from_str::<TokenResponse>(&text)
        .map_err(|e| crate::AuthError::Decode(format!("token response parse failed: {e}")))
}
