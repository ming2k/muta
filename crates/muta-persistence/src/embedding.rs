...
use muta_contracts::{CommandRecord, Message, async_trait};
...
impl EmbeddingStore {
    ...
    async fn save(&self) -> Result<(), String> {
        // Serialise against other `muta` instances in the same project so
...
mod tests {
    ...
    async fn store_indexes_and_searches() {
        let dir = std::env::temp_dir().join(format!("muta-embedding-{}", uuid::Uuid::new_v4()));
        ...
        let mut store = EmbeddingStore::open(dir.join("index.json"), provider)
            ...
            .unwrap();
        let messages = vec![
            Message::new(muta_contracts::Role::User, "how do I reset a password"),
            Message::new(muta_contracts::Role::Assistant, "go to settings"),
        ];
    ...
    async fn store_indexes_and_searches_commands() {
        let dir = std::env::temp_dir().join(format!("muta-embedding-cmd-{}", uuid::Uuid::new_v4()));
        let provider: Arc<dyn EmbeddingProvider> = Arc::new(MockEmbeddingProvider::new(8));
        let mut store = EmbeddingStore::open(dir.join("index.json"), provider)
            .await
            .unwrap();
        let commands = vec![CommandRecord::new("review", "").with_result(
            muta_contracts::CommandResult::Text(
