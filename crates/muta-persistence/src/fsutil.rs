...
fn create_parent_dir(path: &Path) -> std::io::Result<()> {
    muta_platform::secure_file::create_private_parent(path)
}
...
fn create_temporary_file(destination: &Path) -> std::io::Result<(PathBuf, File)> {
    ...
    for _ in 0..1_024 {
        ...
        match muta_platform::secure_file::create_new_private_file(&temporary) {
...
pub fn atomic_write_bytes(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
    ...
    let result = (|| -> std::io::Result<()> {
        ...
        file.sync_all()?;
        drop(file);
        muta_platform::secure_file::atomic_replace(&temporary, path)?;
