...
use neenee_contracts::tokenizer::Tokenizer;
...
fn inline_corpus_matches_reference_exactly() {
    ...
    let cases: &[(&str, &str, usize)] = &[
        ...
        (
            "mixed",
            "在 `crates/neenee-contracts/src/pressure.rs` 中，`count_tokens` 对每个 Unicode 字符分类并加权（CJK ≈ 1.0 token/char，ASCII ≈ 0.25）。",
            54,
        ),
        (
            "code_snip",
