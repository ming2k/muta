...
//! `neenee-contracts` free of dependencies on the concrete state types, which live
...
/// is intentionally no typed list of fields: that would force `neenee-contracts` to
...
///       let cfg = ctx.get::<neenee_contracts::WebSearchConfig>()?.clone();
