...
- **工具调用** — 完整的 ReAct 循环，支持原生与文本回退两种工具调用协议；内置命令执行（execute_command）、文件读写、文件发现（`find_files`）、文本搜索（`search_text`）、网页搜索及 MCP 服务器。
...
> 可用 `MUTA_VERSION=0.35.7` 固定安装此版本，或用 `INSTALL_DIR=/usr/local/bin` 自定义安装目录。
